...
import com.liuhai.wcbox.lock.webview.download.DownloadApkThread;
...
//		}
	...
	public void Download(String url) {
		if(DownloadApkThread.isDownload==true)
			return ;
		Context con=X5WebView.this.getContext();
		try {
			final File filePath = new File(
					Environment.getExternalStorageDirectory() + "/ads12306");
			final String filename = url.substring(url.lastIndexOf("/") + 1);

			File file = new File(filePath, filename);
			if (file.exists()) {
				AppHelp.installDialogInfo(con, filePath, filename);
				return;
			}
			DownloadApkThread downloadThread = new DownloadApkThread(url, filePath, filename, X5WebView.this.getContext());
